...
namespace Lofelt.NiceVibrations
{
    ...
    public static class NiceHaptic
    {
        ...
        static void SetEmphasisFallbackPresetsFromAmplitude(float amplitude)
        {
            if (amplitude > 0.5f)
            {
                HapticController.fallbackPreset = NiceHaptic.PresetType.HeavyImpact;
            }
            else if (amplitude <= 0.5f && amplitude > 0.3)
            {
                HapticController.fallbackPreset = NiceHaptic.PresetType.MediumImpact;
            }
            else if (amplitude <= 0.3f)
            {
                HapticController.fallbackPreset = NiceHaptic.PresetType.LightImpact;
            }
        }
        ...
        public static void PlayHapticClip(HapticClip hapticClip)
        {
            if (!HapticController.hapticsEnabled)
            {
                return;
