...
{
    public class SoldierSelectOpponentSystem : IEcsRunSystem
    {
        ...
        private EcsFilter<CharacterProvider>.Exclude<DeadState, Timer<ReloadingTimer>> _filter;
        private EcsFilter<CharacterProvider, ShootingState>.Exclude<DeadState, Timer<ReloadingTimer>> _shootFilter;
        ...
        public void Run()
        {
            if (!_mineFilter.IsEmpty() || !_shootFilter.IsEmpty() || !_hitFilter.IsEmpty())
                foreach (var idx in _filter)
                {
                    ...
                    ref var shooter = ref entity.Get<CharacterProvider>();
