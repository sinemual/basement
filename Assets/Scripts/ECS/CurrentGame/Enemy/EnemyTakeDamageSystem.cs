...
namespace Client.ECS.CurrentGame.Mining
{
    public class EnemyTakeDamageSystem : IEcsRunSystem
    {
        ...
        private EcsFilter<EnemyProvider, HitRequest>.Exclude<DeadState> _requestfilter;
        ...
        public void Run()
        {
            foreach (var idx in _requestfilter)
            {
                ...
                if (_data.PlayerData.Equipment[EquipType.Bow] > 0)
        ...
        {
            EcsEntity spawnEntity = _prefabFactory.Spawn(_data.StaticData.PrefabData.MiningAnimViewPrefabs[EquipType.Bow],
            ...
            spawnEntity.Get<MiningAnimViewProvider>().ToolImage.sprite =
                _playerFilter.GetEntity(0).Get<Equipment>().Value[EquipType.Bow].View.ItemSprite;
